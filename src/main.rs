...
struct EmitterRgx {
    ...
    backtick_rgx: regex::Regex,
    bracket_left_rgx: regex::Regex,
    bracket_right_rgx: regex::Regex,
    star_rgx: regex::Regex,
    underscore_rgx: regex::Regex
}
...
impl EmitterRgx {
    fn new() -> EmitterRgx {
        ...
        let backtick_rgx = Regex::new(r"`").unwrap();
        let bracket_left_rgx = Regex::new(r"\[").unwrap();
        let bracket_right_rgx = Regex::new(r"\]").unwrap();
        let star_rgx = Regex::new(r"\*").unwrap();
        let underscore_rgx = Regex::new(r"_").unwrap();
    ...
    #[inline]
    fn tg_escape_string(&self, orig: String) -> String {
        let orig = orig.clone();

        let orig_pr = self.backtick_rgx.replace_all(&orig, "\\`").to_string();
        let orig_pr = self.bracket_left_rgx.replace_all(&orig_pr, "\\[").to_string();
        let orig_pr = self.bracket_right_rgx.replace_all(&orig_pr, "\\]").to_string();
        let orig_pr = self.star_rgx.replace_all(&orig_pr, "\\*").to_string();

        self.underscore_rgx.replace_all(&orig_pr, "\\*").to_string()
    }
}
...
impl JiraEmitter {
    ...
    fn _get_formatted_event(&self, event: JiraEvent, event_type: JiraEventTypes) -> String {
        format!(
r#"Issue <a href="{}">{}</a> was {} by <a href="{}">{}</a>
<b>{}</b>

Priority: <b>{}</b>"#,
            self.emitter_rgx.tg_escape_string(event.issue._self),
            self.emitter_rgx.tg_escape_string(event.issue.key),
            self._get_verb_from_type(event_type),
            self.emitter_rgx.tg_escape_string(event.user._self),
            self.emitter_rgx.tg_escape_string(event.user.name),
            self.emitter_rgx.tg_escape_string(event.issue.fields.summary),
            self.emitter_rgx.tg_escape_string(event.issue.fields.priority)
        )
    }
